...
const FloatingBackButton = () => {
  ...
  return (
    <View style={styles.BackButton}>
      <View onTouchEnd={() => navigate("/chats")}>
...
const PersonBanner = ({ user }: { user: any }) => {
  ...
  return (
    <View style={styles.PictureContainer}>
      <Image
        source={{
          uri: `data:image/${user.pictures[0].extension};base64,${user.pictures[0].file}`,
        }}
...
const Chat = () => {
  ...
  useEffect(() => {
    const getWebSocket = async () => {
      ...
      ws.current = new WebSocket(`ws://192.168.0.103:8000/ws/user/${token}/`);
      ws.current.onopen = () => {
  ...
  const SendMessage = async () => {
    const token = await SecureStore.getItemAsync("token");
    const apiUrl =
      constants.manifest!.extra!.apiUrl + "api/users/send_message/";
    ...
    const destinedUser = chat.users.filter(
      (user: any) => user.id !== response.data.user
    )[0];
    ws.current.send(
      JSON.stringify({
        user_dest: destinedUser.id,
        ...response.data,
      })
    );
